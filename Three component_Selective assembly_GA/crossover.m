...
for n=1:2:ncross 
    ...
    child1rand3=rand(1,n_sa);
    k13=sort(child1rand3);
    for i=1:n_sa
        A13=find(i==p1(2.*n_sa+1:end),1);
        child1sorted(A13+2.*n_sa)=k13(i);
    end
    ...
    child2rand3=rand(1,n_sa);
    k23=sort(child2rand3);
    for i=1:n_sa
        A23=find(i==p2(2.*n_sa+1:end),1);
        child2sorted(A23+2.*n_sa)=k23(i);
    end
    %
%%
    ...
    for i=1:n_sa
        ...
        k3=randi([2.*n_sa+1 3.*n_sa-1]);

        o1=[child1sorted(1:k1) child2sorted(k1+1:n_sa) child1sorted(n_sa+1:k2) child2sorted(k2+1:2.*n_sa) child1sorted(2.*n_sa+1:k3) child2sorted(k3+1:end)];
        o2=[child2sorted(1:k1) child1sorted(k1+1:n_sa) child2sorted(n_sa+1:k2) child1sorted(k2+1:2.*n_sa) child2sorted(2.*n_sa+1:k3) child1sorted(k3+1:end)];

        [yadola, poppar]=sort(o1(1:n_sa));
        [yadola, poppar(n_sa+1:2.*n_sa)]=sort(o1(n_sa+1:2.*n_sa));
        [yadola, poppar(2.*n_sa+1:3.*n_sa)]=sort(o1(2.*n_sa+1:3.*n_sa));
        ...
        [yadola, poppar2(2.*n_sa+1:3.*n_sa)]=sort(o2(2.*n_sa+1:3.*n_sa));
